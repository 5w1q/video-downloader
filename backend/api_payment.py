...
def _default_frontend_url() -> str:
    env = (_get_config("APP_ENV") or _get_config("ENV") or "").strip().lower()
...
async def create_checkout_session(req: CreateCheckoutRequest, user: dict = Depends(get_current_user)):
    raise HTTPException(
        status_code=410,
        detail="支付已统一迁移至 Ab 主站，请在 Ab 主站账户页完成开通/充值。",
    )

    secret_key = _get_config("STRIPE_SECRET_KEY")
    price_id = _get_config("STRIPE_PRICE_ID_MONTHLY")
    frontend_url = _get_config("FRONTEND_URL", _default_frontend_url())
...
async def list_orders(user: dict = Depends(get_current_user)):
    raise HTTPException(
        status_code=410,
        detail="订单已统一迁移至 Ab 主站，请在 Ab 主站账户页查看。",
    )
