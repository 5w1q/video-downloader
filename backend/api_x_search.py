"""X（Twitter）关键词搜索 + 阈值筛选，并可直接接入批量下载 SSE。"""
...
class XSearchRequest(BaseModel):
    ...
    videos_only: bool = Field(True)


@router.post("/x/search")
async def x_search(body: XSearchRequest):
    """仅搜索并筛选，返回候选列表（不下载）。"""
    ...
    try:
        result = await loop.run_in_executor(
            ...
            partial(
                ...
                min_retweets=body.min_retweets,
...
@router.post("/x/search-download")
async def x_search_download(
    query: str = Form(...),
    max_results: int = Form(20),
    min_likes: int = Form(0),
    min_retweets: int = Form(0),
    ...
    do_videos = str(videos_only).lower() in ("1", "true", "yes", "on")
    ...
    async def event_stream() -> AsyncIterator[str]:
        ...
        async for chunk in run_bulk_download_stream(
            ...
            format_id=format_id,
            delay=delay,
            source_name=f"x:{found.get('query', query)}",
            start_extra={
                "platform": "x",
                "query": found.get("query"),
                "min_likes": min_likes,
