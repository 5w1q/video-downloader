...
# 进程内共用 Session：分享页 WAF 会种 Cookie；解析与 AI 总结各 new 一个 Parser 时若不复用 Session，
# 总结阶段容易重复被风控。共用 Session 可提高成功率。
_douyin_http_session: requests.Session | None = None


def _shared_douyin_session() -> requests.Session:
    global _douyin_http_session
    if _douyin_http_session is None:
        s = requests.Session()
        s.headers.update(DEFAULT_HEADERS)
        _douyin_http_session = s
    return _douyin_http_session
...
class DouyinParser:
    ...
    def __init__(self, download_dir: str = "downloads"):
        ...
        self.session = _shared_douyin_session()
        self.timeout = (15, 45)
        self.max_retries = 3

    @staticmethod
    def _json_from_response(resp: requests.Response, label: str) -> dict:
        raw = resp.content or b""
        stripped = raw.strip()
        if not stripped:
            raise ValueError(
                f"{label}：响应体为空（无 Cookie 时常见，将尝试其它方式）"
            )
        if stripped[:1] == b"<":
            raise ValueError(
                f"{label}：返回 HTML 而非 JSON（风控页常见），将尝试其它方式"
            )
        try:
            ...
            raise ValueError(
                f"{label}：无法解析为 JSON（{e}）。开头: {head!r}"
            ) from e
        if not isinstance(data, dict):
            raise ValueError(f"{label}：JSON 根为 {type(data).__name__}，期望对象")
        return data
    ...
    def _fetch_item_info(self, video_id: str, resolved_url: str) -> dict:
        ...
        for label, fn in (
            ...
            try:
                ...
                last_err = e
                logger.debug("抖音元数据 %s 未成功: %s", label, e)
    ...
    def _fetch_via_web_detail(self, video_id: str) -> dict:
        ...
        resp.raise_for_status()
        data = self._json_from_response(resp, "web_aweme_detail")
    ...
    def _fetch_via_api(self, video_id: str) -> dict:
        ...
        for attempt in range(self.max_retries):
            try:
                ...
                resp.raise_for_status()
                data = self._json_from_response(resp, "legacy_iteminfo")
