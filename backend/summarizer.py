...
class SubtitleExtractor:
    ...
    def extract(self, url: str) -> dict:
        ...
        try:
            ...
            alt = try_paraformer_transcribe(url)
            if alt and alt.get("has_subtitle"):
                return alt
        except Exception:
            pass
        return out

    def _extract_inner(self, url: str) -> dict:
        """
        提取视频字幕，返回:
        {
            "has_subtitle": bool,
        ...
        if _is_bilibili_url(url):
            ...
            if result["has_subtitle"]:
                return result
            # 无创作者/CC 字幕后走下方 yt-dlp（B 站 extractor 仍可能提供 automatic_captions 等）

        if is_douyin_url(url):
            douyin_out = self._extract_douyin(url)
            if douyin_out.get("has_subtitle"):
                return douyin_out
            # 默认不启用：Douyin extractor 常需 Cookie；设 DOUYIN_TRY_YTDLP_SUBTITLES=1 时再尝试 yt-dlp
            if _env_truthy("DOUYIN_TRY_YTDLP_SUBTITLES"):
                ytdlp_out = self._extract_via_ytdlp(url)
                if ytdlp_out.get("has_subtitle"):
                    return ytdlp_out
            return douyin_out

        return self._extract_via_ytdlp(url)

    def _extract_via_ytdlp(self, url: str) -> dict:
        """
        使用 yt-dlp 的 ``subtitles`` / ``automatic_captions`` 元数据取字幕；
        与 ``yt-dlp --skip-download --write-subs --write-auto-subs <URL>`` 同源数据路径。
        """
        ...
        if not sub_url:
            # Twitter/X 多数视频无人工/自动字幕；若有解析出的标题与推文正文，用作弱文本仍可走 AI 总结
            tw_fallback = self._twitter_text_fallback(info, url)
            if tw_fallback:
                return {
                    "has_subtitle": True,
                    "language": "en",
    ...
    @staticmethod
    def _twitter_text_fallback(info: dict, url: str) -> str:
        if not _is_twitter_or_x_url(url):
            return ""
        parts: list[str] = []
...
def _resolve_summarize_llm_settings() -> tuple[str, str, str]:
    """
    返回 (base_url, api_key, model)，供 OpenAI SDK 调用。
    优先级：SUMMARIZE_LLM_*（阿里云百炼兼容模式等）> DEEPSEEK_*（沿用旧配置）。
    """
    key = (os.getenv("SUMMARIZE_LLM_API_KEY") or "").strip()
    if key:
        base = (
            (os.getenv("SUMMARIZE_LLM_BASE_URL") or "").strip()
            or "https://dashscope.aliyuncs.com/compatible-mode/v1"
        ).rstrip("/")
        model = ((os.getenv("SUMMARIZE_LLM_MODEL") or "").strip() or "qwen-plus")
        return base, key, model
    key = (os.getenv("DEEPSEEK_API_KEY") or "").strip()
    if key:
        base = ((os.getenv("DEEPSEEK_BASE_URL") or "").strip() or "https://api.deepseek.com").rstrip("/")
        model = ((os.getenv("DEEPSEEK_MODEL") or "").strip() or "deepseek-chat")
        return base, key, model
    raise ValueError(
        "未配置 AI 总结/问答：请设置 SUMMARIZE_LLM_API_KEY（推荐阿里云 DashScope 兼容 OpenAI），"
        "或保留 DEEPSEEK_API_KEY。详见 backend/.env.example。"
    )


def summarize_llm_configured() -> bool:
    try:
        _resolve_summarize_llm_settings()
        return True
    except ValueError:
        return False
...
class VideoSummarizer:
    """使用 OpenAI 兼容 Chat Completions API 生成视频总结、思维导图、问答"""

    def __init__(self):
        base, api_key, model = _resolve_summarize_llm_settings()
