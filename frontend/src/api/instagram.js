/**
 * Instagram 关键词搜索 + 阈值筛选；可选直接走 bulk SSE 下载。
 */
...
/**
 * 仅搜索，不下载。
 * @param {{ query: string, maxResults?: number, minViews?: number, minLikes?: number, minComments?: number, searchPool?: number, dateFilter?: string, uploadDate?: string }} opts
 */
export async function searchInstagram(opts = {}) {
  const {
    query,
    maxResults = 20,
    minViews = 0,
 ...
 */
export async function instagramSearchDownloadStream(options = {}) {
  const {
    query,
    maxResults = 20,
    minViews = 0,
  ...
  const res = await fetch('/api/instagram/search-download', {
    method: 'POST',
    body: form,
    signal,
  })
