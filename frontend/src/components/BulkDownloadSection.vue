    ...
    <div class="max-w-3xl mx-auto px-4 sm:px-6">
      <div class="text-center mb-8">
        ...
        <p class="text-text-secondary text-sm sm:text-base max-w-xl mx-auto">
          上传 Excel（.xlsx）、CSV 或 TXT，自动识别链接并顺序下载。
        </p>
      </div>
      ...
      <div class="bg-white rounded-2xl border border-border-light shadow-sm p-5 sm:p-6 space-y-5">
        ...
        <div class="flex flex-col sm:flex-row sm:flex-wrap gap-4 text-sm">
          <label class="inline-flex items-center gap-2 cursor-pointer select-none">
            <input v-model="packForBrowser" type="checkbox" :disabled="running" class="rounded border-border text-primary focus:ring-primary/30" />
            <span class="text-text-secondary">完成后下载 ZIP 到本机</span>
          </label>
          <label class="inline-flex items-center gap-2 cursor-pointer select-none">
...
/**
 * 用同源直链触发浏览器原生下载：走系统/浏览器设置的默认下载目录，且不把整包 ZIP 读入 JS 内存。
 */
function downloadZipFromUrl(urlPath) {
    ...
    await bulkDownloadStream(file, {
      ...
      onEvent: (data) => {
        ...
        if (ev === 'start') {
          ...
          if (data.zip_url && (data.ok ?? 0) > 0) {
            try {
              downloadZipFromUrl(data.zip_url)
            } catch (err) {
              pushLog(`ZIP 下载触发失败：${err.message || err}`)
            }
          }
        } else if (ev === 'error') {
