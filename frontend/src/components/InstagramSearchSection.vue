<template>
  <component
    ...
    :id="embedded ? undefined : 'instagram-search'"
    :class="embedded ? '' : 'py-12 sm:py-16 bg-white border-t border-border-light'"
    :aria-labelledby="embedded ? undefined : 'instagram-search-heading'"
  >
    <div :class="embedded ? '' : 'max-w-3xl mx-auto px-4 sm:px-6'">
      <div v-if="!embedded" class="text-center mb-8">
        <h2 id="instagram-search-heading" class="text-2xl sm:text-3xl font-bold text-text-primary">
          Instagram 关键词下载
        </h2>
      </div>
      ...
      <div class="bg-bg-section rounded-2xl border border-border-light shadow-sm p-5 sm:p-6 space-y-5">
        <div>
          ...
          <input
            ...
            type="text"
            maxlength="200"
            placeholder="例如：travel、cooking、AI"
            :disabled="running"
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div>
            <label class="block text-xs font-medium text-text-muted mb-1">评论 ≥</label>
            <input
          ...
          <ul v-if="previewResults.length" class="max-h-56 overflow-y-auto divide-y divide-border/60 text-sm">
            <li v-for="item in previewResults" :key="item.id || item.url" class="px-3 py-2.5">
              ...
              <p class="mt-0.5 text-xs text-text-muted">
                ...
                · 评论 {{ formatCount(item.comment_count) }}
          ...
          <ul class="space-y-2 max-h-64 overflow-y-auto">
            ...
            >
              ...
              <a
                ...
                :download="`instagram-batch-part${String(part.part).padStart(2, '0')}.zip`"
              >
                备用链接
              </a>
            </li>
...
import { searchInstagram, instagramSearchDownloadStream } from '../api/instagram.js'
...
const previewDownloadUrls = computed(() =>
  (previewResults.value || [])
    .filter((item) => typeof item?.url === 'string' && /^https?:\/\//i.test(item.url))
    .map((item) => {
      const vu = typeof item?.video_url === 'string' ? item.video_url.trim() : ''
      return /^https?:\/\//i.test(vu) ? vu : item.url
    })
)
...
watch([query, maxResults, minViews, minLikes, minComments, dateFilter, uploadDate], () => {
...
function startBrowserZipDownload(url, filename = 'instagram-batch.zip') {
...
function scrollToZipActions() {
  nextTick(() => {
    document.getElementById('instagram-zip-actions')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
  })
}
...
function onDownloadZipPart(part) {
  if (!part?.url) return
  try {
    const name = `instagram-batch-part${String(part.part).padStart(2, '0')}.zip`
...
async function downloadPreview() {
  ...
  try {
    await bulkDownloadUrlsStream(urls, {
      ...
      sourceName: `instagram-preview:${query.value.trim() || 'results'}`,
      titles: previewTitles.value,
      downloadUrls: previewDownloadUrls.value,
...
async function startSearchAndDownload() {
  ...
  try {
    await instagramSearchDownloadStream({
      query: q,
      maxResults: maxResults.value || 10,
      minViews: minViews.value || 0,
      minLikes: minLikes.value || 0,
