        ...
        <div class="p-5 sm:p-6 min-h-[400px] flex-1 overflow-y-auto">
          ...
          <div v-show="activeTab === 'summary'">
            ...
            <div v-if="quotaInfo && quotaInfo.remaining >= 0 && !loading" class="mt-4 p-3 rounded-xl bg-blue-50 border border-blue-100">
              ...
              <p
                v-if="quotaInfo.remaining === 0 && user && !user.is_vip"
                class="mt-2 text-xs text-amber-800 border-t border-amber-100 pt-2"
              >
                免费次数今日已用尽。可明天再试，或
                <button type="button" class="text-primary font-medium underline cursor-pointer" @click="goPricing">
                  开通 VIP
                </button>
                解锁无限次 AI 总结。
              </p>
...
import { summarizeVideo, chatWithVideo } from '../api/videoSummarizeApi.js'
    ...
    await summarizeVideo(props.videoUrl, 'zh', {
      progress: (data) => {
        try {
          ...
          if (p.message) loadingMessage.value = p.message
        } catch {
          /* ignore */
        }
      },
