<template>
  <component
    ...
    :id="embedded ? undefined : 'x-search'"
    :class="embedded ? '' : 'py-12 sm:py-16 bg-white border-t border-border-light'"
    :aria-labelledby="embedded ? undefined : 'x-search-heading'"
  >
    <div :class="embedded ? '' : 'max-w-3xl mx-auto px-4 sm:px-6'">
      <div v-if="!embedded" class="text-center mb-8">
        <h2 id="x-search-heading" class="text-2xl sm:text-3xl font-bold text-text-primary">
          X 关键词下载
        </h2>
      </div>
      ...
      <div class="bg-bg-section rounded-2xl border border-border-light shadow-sm p-5 sm:p-6 space-y-5">
        ...
        <div class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-4">
          ...
          <div>
            <label class="block text-xs font-medium text-text-muted mb-1">转发 ≥</label>
            <input
              v-model.number="minRetweets"
              type="number"
              min="0"
              step="10"
              placeholder="0 表示不限"
          ...
          <ul class="space-y-2 max-h-64 overflow-y-auto">
            ...
            >
              ...
              <a
                ...
                :download="`x-batch-part${String(part.part).padStart(2, '0')}.zip`"
              >
                备用链接
              </a>
            </li>
...
watch([query, maxResults, minLikes, minRetweets, minComments, dateFilter, uploadDate], () => {
...
function startBrowserZipDownload(url, filename = 'x-batch.zip') {
...
function scrollToZipActions() {
  nextTick(() => {
    document.getElementById('x-zip-actions')?.scrollIntoView({ behavior: 'smooth', block: 'nearest' })
  })
}
...
function onDownloadZipPart(part) {
  if (!part?.url) return
  try {
    const name = `x-batch-part${String(part.part).padStart(2, '0')}.zip`
